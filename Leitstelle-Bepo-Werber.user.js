...
// @downloadURL  https://raw.githubusercontent.com/NilsPee/LSS_V2_Scripts/main/Leitstelle-Bepo-Werber.user.js
// @updateURL    https://raw.githubusercontent.com/NilsPee/LSS_V2_Scripts/main/Leitstelle-Bepo-Werber.user.js
...
(async function () {
  ...
  async function loadConfiguration() {
    ...
    return {
      ...
      policeReserve:
        Math.max(
          await numberValue(KEYS.policeReserve, 36),
          2
        ),

      bepoReserve:
        Math.max(
          await numberValue(KEYS.bepoReserve, 244),
          0
        ),

      requestDelay:
        Math.max(
          await numberValue(KEYS.requestDelay, 100),
          0
        ),

      excludedTargets:
        new Set(
          await storedArray(KEYS.excludedTargets)
        ),

      excludedSources:
        new Set(
          await storedArray(KEYS.excludedSources)
        ),

      excludedDispatchCenters:
        new Set(
          await storedArray(KEYS.excludedDispatchCenters)
        ),

      limitedDispatchCenters:
        new Set(
  ...
  function selectTargets(buildings, configuration, context) {
    return buildings
      .filter(building =>
        building.building_type === 11
      )
      .filter(building =>
        !configuration.excludedTargets.has(
          building.id
        )
      )
      .filter(building => {
        ...
        return building.leitstelle_building_id ===
          context.dispatchCenterId;
      })
      .map(building => ({
        ...building,
  ...
  function selectSources(buildings, configuration) {
    const limited =
      configuration.limitedDispatchCenters.size > 0;

    return buildings
      .filter(building =>
        !configuration.excludedSources.has(
          building.id
        )
      )
      .filter(building =>
      ...
      .map(building => {
        ...
        return {
          ...
          available:
            Math.max(
              building.personal_count - reserve,
              0
            )
        };
      })
  ...
  function untrainedPersonnelIds(
    documentFromResponse,
    sourceBuildingId
  ) {
    const table =
      documentFromResponse.getElementById(
        `personal_table_${sourceBuildingId}`
      ) ??
    ...
    for (
      const row
      of table.querySelectorAll('tbody tr')
    ) {
      const cells =
  ...
  async function livePersonnelCount(buildingId) {
    ...
    if (!response.ok) {
      throw new Error(
        `Personalzaehlung HTTP ${response.status}`
      );
    }
    ...
    return personnelCountFromDocument(
      documentFromResponse,
      buildingId
    );
  }

  async function refreshTargetNeed(
    target,
    configuration
  ) {
    try {
      const liveCount =
        await livePersonnelCount(
          target.id
        );
  ...
  async function availablePersonnelIds(
    ...
    if (!response.ok) {
      throw new Error(
        `${sourceBuilding.caption}: ` +
        `Personalabfrage HTTP ${response.status}`
      );
    }
    ...
    return personnelIds.slice(
      0,
      allowed
    );
  }

  async function adoptPersonnel(
    targetBuildingId,
    personnelIds
  ) {
    const token =
    ...
    const response =
      await fetch(
        `/buildings/${targetBuildingId}/adopt`,
        {
          method:
            'POST',
  ...
  async function runRecruiter() {
    try {
      ...
      if (
        configuration.targetPersonnel <= 0
      ) {
        throw new Error(
          'Das Zielpersonal ist nicht groesser als 0.'
        );
      }
      ...
      if (
        ...
        await stopRun(
          'Keine BePo-Wache muss aufgefuellt werden',
          'success'
        );
      ...
      if (
        ...
        throw new Error(
          'Kein Personal oberhalb der eingestellten Reserven.'
        );
      }
